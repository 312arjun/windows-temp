...
:build
	...
	set ECLIPZ_VERSION=%ECLIPZ_VERSION:"=%
...
:sign
	...
	echo [+] Signing
	signtool sign /sha1 "%SigningCertificate%" /fd sha256 /tr "%TimestampServer%" /td sha256 /d Eclipz x86\eclipz.exe x86\wg.exe amd64\eclipz.exe amd64\wg.exe arm64\eclipz.exe arm64\wg.exe || goto :error

:success
	echo [+] Success. Launch eclipz.exe.
	exit /b 0
...
:build_plat
	...
	go build -tags load_wgnt_from_rsrc -ldflags="-H windowsgui -s -w" -trimpath -buildvcs=false -v -o "%~1\eclipz.exe" || exit /b 1
