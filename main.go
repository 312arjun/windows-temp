...
func checkForWow64() {
	...
	if err != nil {
		tunnel.WGlog("Unable to determine whether the process is running under WOW64: %v", err)
		fatalf("Unable to determine whether the process is running under WOW64: %v", err)
	}
	if b {
		tunnel.WGlog("You must use the native version of WireGuard on this computer.")
...
func checkForAdminGroup() {
	...
	if !elevate.TokenIsElevatedOrElevatable(processToken) {
		fatalf("Eclipz may only be used by users who are a member of the Builtin %s group.", elevate.AdminGroupName())
	}
}
...
func checkForAdminDesktop() {
	...
	if !adminDesktop && err == nil {
		fatalf("Eclipz is running, but the UI is only accessible from desktops of the Builtin %s group.", elevate.AdminGroupName())
	}
}
...
func main() {
	defer func() {
		if r := recover(); r != nil {
			tunnel.WGlog("EERROORR: %v", r)
		}
	}()
	tunnel.WGlog("main: ENTRY; args %+v", os.Args)

	// Check if config.json exists
	if !manager.CheckConfigFile() {
		cw, err := ui.NewConfigWindow()
		if err != nil {
			log.Fatal(err)
		}
	...
	if len(os.Args) <= 1 {
		tunnel.WGlog("main: no args, ui.RaiseUI")
		if ui.RaiseUI() {
			return
		}
		checkForAdminGroup()
	...
	case "/installmanagerservice":
		...
		tunnel.WGlog("main: /installmanagerservice, starting go WaitForRaiseUIThenQuit()")
		...
		//fatalf("Eclipz system tray icon did not appear after 30 seconds.")
		tunnel.WGlog("main: /installmanagerservice : Eclipz system tray icon did not appear after 30 seconds.")
		return
	...
	case "/managerservice":
		...
		tunnel.WGlog("main: /managerservice, calling manager.Run()")
		err := manager.Run()
		if err != nil {
			fatal(err)
		}
	...
	case "/installtunnelservice":
		tunnel.WGlog("main: /installtunnelservice, tunnel :%s", os.Args[2])
		if len(os.Args) != 3 {
			usage()
		}
		err := manager.InstallTunnel(os.Args[2])
		tunnel.WGlog("main: /installtunnelservice done, err:%s", err)
		if err != nil {
			fatal(err)
		}
		return
	case "/uninstalltunnelservice":
		tunnel.WGlog("main: /uninstalltunnelservice")
		if len(os.Args) != 3 {
			usage()
		}
	...
	case "/tunnelservice":
		...
		tunnel.WGlog("main: /tunnelservice, tunnel: %s", os.Args[2])
		err := tunnel.Run(os.Args[2])
		if err != nil {
			tunnel.WGlog("main: tunnel.Run() error: %s", err)
			fatal(err)
		}
		tunnel.WGlog("main: /tunnelservice, done")
		return
	case "/ui":
		if len(os.Args) != 6 {
			usage()
		...
		tunnel.WGlog("IsAdmin = %v", isAdmin)
		if isAdmin {
		...
		tunnel.WGlog("main: /ui, calling InstallTunnelByName()")
		go manager.InstallTunnelByName("eclipz0")

		// Not really sure how long this sleep should be. Ideally we should just
		// wait until the tunnel appears (with a tmo limit ~5sec)  and then go forward.
		time.Sleep(time.Second * 3)

		tunnel.WGlog("main: /managerservice, calling eclipz.EclipzInit()")
		eclipz.EclipzInit()
		tunnel.WGlog("main: /managerservice, launching eclipz.EclipzMain()")
		go eclipz.EclipzMain()

		ui.IsAdmin = isAdmin
		tunnel.WGlog("main: /ui, calling ui.RunUI()")
		ui.RunUI()
		tunnel.WGlog("main: /ui, done")
		return
	case "/dumplog":
