...
:build
	...
	echo [+] Signing
	signtool sign /sha1 "%SigningCertificate%" /fd sha256 /tr "%TimestampServer%" /td sha256 /d "Eclipz Setup" "dist\wireguard-*-%WIREGUARD_VERSION%.msi" || goto :error
...
:msi
	...
	echo [+] Signing %1
	signtool sign /sha1 "%SigningCertificate%" /fd sha256 /tr "%TimestampServer%" /td sha256 /d "Eclipz Agent Setup Custom Actions" "%~1\customactions.dll" || exit /b 1
:skipsign
	"%WIX%bin\candle" %WIX_CANDLE_FLAGS% -dWIREGUARD_PLATFORM="%~1" -out "%~1\eclipz.wixobj" -arch %3 eclipz.wxs || exit /b %errorlevel%
	echo [+] Linking %1
	"%WIX%bin\light" %WIX_LIGHT_FLAGS% -out "dist\eclipz-%~1-%WIREGUARD_VERSION%.msi" "%~1\eclipz.wixobj" -ext WixUIExtension -ext WixUtilExtension || exit /b %errorlevel%
	goto :eof
