...
#define MANAGER_SERVICE_NAME TEXT("EclipzManager")
#define TUNNEL_SERVICE_PREFIX TEXT("EclipzTunnel$")
...
static UINT insert_service_control(MSIHANDLE installer, MSIHANDLE view, const TCHAR *service_name, bool start)
{
	...
	MsiRecordSetString (record, 4/*Component_    */, TEXT("EclipzExecutable"));
...
__declspec(dllexport) UINT __stdcall LaunchApplicationAndAbort(MSIHANDLE installer)
{
	...
	ret = MsiGetProperty(installer, TEXT("EclipzFolder"), path, &path_len);
	if (ret != ERROR_SUCCESS) {
		log_errorf(installer, LOG_LEVEL_WARN, ret, TEXT("MsiGetProperty(\"EclipzFolder\") failed"));
		goto out;
	}
	if (!path[0] || !PathAppend(path, TEXT("exlipz.exe")))
		goto out;
	log_messagef(installer, LOG_LEVEL_INFO, TEXT("Launching %1"), path);
	if (!CreateProcess(path, TEXT("exlipz"), NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
...
__declspec(dllexport) UINT __stdcall EvaluateEclipzComponents(MSIHANDLE installer)
{
	UINT ret = ERROR_INSTALL_FAILURE;
	bool is_com_initialized = SUCCEEDED(CoInitialize(NULL));
	...
	ret = MsiGetComponentState(installer, TEXT("EclipzExecutable"), &component_installed, &component_action);
	if (ret != ERROR_SUCCESS) {
		log_errorf(installer, LOG_LEVEL_ERR, ret, TEXT("MsiGetComponentState(\"EclipzExecutable\") failed"));
		goto out;
	}
	ret = MsiGetProperty(installer, TEXT("EclipzFolder"), path, &path_len);
	if (ret != ERROR_SUCCESS) {
...
__declspec(dllexport) UINT __stdcall KillWireGuardProcesses(MSIHANDLE installer)
{
	...
	if (PathCombine(executable, process_path, TEXT("wg.exe")) && calculate_file_id(executable, &file_ids[file_ids_len]))
		++file_ids_len;
	if (PathCombine(executable, process_path, TEXT("exlipz.exe")) && calculate_file_id(executable, &file_ids[file_ids_len]))
		++file_ids_len;
	if (!file_ids_len)
		goto out;
	...
	for (bool ret = Process32First(snapshot, &entry); ret; ret = Process32Next(snapshot, &entry)) {
		if (_tcsicmp(entry.szExeFile, TEXT("exlipz.exe")) && _tcsicmp(entry.szExeFile, TEXT("wg.exe")))
			continue;
...
__declspec(dllexport) UINT __stdcall RemoveAdapters(MSIHANDLE installer)
{
	...
	if (!CreateProcess(path, TEXT("exlipz /removedriver"), NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
		log_errorf(installer, LOG_LEVEL_WARN, GetLastError(), TEXT("Failed to create \"%1\" process"), path);
		goto cleanup_pipe_w;
	}
