...
func (service *managerService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (svcSpecificEC bool, exitCode uint32) {
	...
	tunnel.WGlog("managerService: starting...")
	var err error
	...
	startProcess := func(session uint32) {
		...
		for {
			...
			tunnel.WGlog("managerService: Starting UI process for user ‘%s@%s’ for session %d", username, domain, session)
			...
			if exitCode, err := proc.Wait(); err == nil {
				tunnel.WGlog("managerService: proc.Wait; exitCode %x", exitCode)
				...
				tunnel.WGlog("managerService: Unable to wait for UI process for user '%s@%s' for session %d: %v",
					username, domain, session, err)
	...
	tunnel.WGlog("managerService: started")
...
loop:
	for {
		...
		case <-quitManagersChan:
			uninstall = true
			tunnel.WGlog("managerService: quitManagersChan")
			break loop
		case c := <-r:
			switch c.Cmd {
			case svc.Stop:
				tunnel.WGlog("managerService: svc.Stop")
				break loop
	...
	tunnel.WGlog("managerService: after for loop")
...
func Run() error {
	tunnel.WGlog("svc.Run managerService")
	return svc.Run("EclipzManager", &managerService{})
}
