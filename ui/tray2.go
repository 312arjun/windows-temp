...
const trayTunnelActionsOffset2 = 3
...
func NewTray2(mtw *AgentWindow) (*Tray2, error) {
	var err error
...
func (tray *Tray2) setup() error {
	tray.clicked = tray.onManageTunnels

	tray.SetToolTip(l18n.Sprintf("Eclipz: Deactivated"))
	...
	for _, item := range [...]struct {
		...
		{separator: true},
		{label: l18n.Sprintf("&About Eclipz…"), handler: tray.onAbout, enabled: true},
...
func (tray *Tray2) onTunnelsChange() {
...
func (tray *Tray2) sortedTunnels() []string {
	var names []string
...
func (tray *Tray2) addTunnelAction(tunnel *manager.Tunnel) {
...
func (tray *Tray2) removeTunnelAction(tunnelName string) {
	if tray.tunnelsAreInBreakoutMenu {
...
func (tray *Tray2) rebalanceTunnelsMenu() {
...
func (tray *Tray2) onTunnelChange(tunnel *manager.Tunnel, state, globalState manager.TunnelState, err error) {
	tray.mtw.Synchronize(func() {
		tray.updateGlobalState(globalState)
		if err == nil {
			...
			if tunnelAction != nil {
				...
				case manager.TunnelStarted:
					if !wasChecked {
						...
						tray.ShowCustom(l18n.Sprintf("Eclipz Activated"), l18n.Sprintf("The %s tunnel has been activated.", tunnel.Name), icon)
					}

				case manager.TunnelStopped:
					if wasChecked {
						icon, _ := loadSystemIcon("imageres", -31, 128) // TODO: this icon isn't very good...
						tray.ShowCustom(l18n.Sprintf("Eclipz Deactivated"), l18n.Sprintf("The %s tunnel has been deactivated.", tunnel.Name), icon)
					}
				}
			}
		} else if !tray.mtw.Visible() {
			tray.ShowError(l18n.Sprintf("Eclipz Tunnel Error"), err.Error())
		}
		tray.setTunnelState(tunnel, state)
	})
}

func (tray *Tray2) updateGlobalState(globalState manager.TunnelState) {
	...
	tray.SetToolTip(l18n.Sprintf("Eclipz: %s", textForState(globalState, true)))
...
func (tray *Tray2) setTunnelState(tunnel *manager.Tunnel, state manager.TunnelState) {
	tunnelAction := tray.tunnels[tunnel.Name]
	if tunnelAction == nil {
		return
	}
...
func (tray *Tray2) UpdateFound() {
	...
	showUpdateBalloon := func() {
		...
		tray.ShowCustom(l18n.Sprintf("Eclipz Update Available"), l18n.Sprintf("An update to Eclipz is now available. You are advised to update as soon as possible."), icon)
	}

	timeSinceStart := time.Since(startTime) //  time.Now().Sub(startTime)
...
func (tray *Tray2) onManageTunnels() {
	// tray.mtw.tunnelsPage.listView.SelectFirstActiveTunnel()
	// tray.mtw.tabs.SetCurrentIndex(0)
	raise(tray.mtw.Handle())
}

func (tray *Tray2) onAbout() {
	if tray.mtw.Visible() {
		onAbout(tray.mtw)
	} else {
		onAbout(nil)
